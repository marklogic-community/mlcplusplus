//
//  ResponseCodes.h
//  Scratch
//
//  Created by Paul Hoehne on 5/30/14.
//  Copyright (c) 2014 Paul Hoehne. All rights reserved.
//

#ifndef RESPONSECODES_H
#define RESPONSECODES_H

#include "mlclient.hpp"
#include <cstdint>
#include <string>
#include <iostream>


namespace mlclient {

///
/// The type of response
///
//enum ResponseType : int; // UNKNOWN IS FIRST SO AS TO AVOID INCORRECTLY INSTANTIATED RESPONSE ISSUES

enum ResponseType : int { UNKNOWN_TYPE = 0, XML = 1, JSON = 2, TEXT = 3, BINARY = 4 };

std::ostream& operator << (std::ostream& os, const ResponseType& rt);
std::string& operator +(std::string& s, const ResponseType& rt);
//std::string& operator+(const char* orig,const ResponseType& rt);

const std::string translate(const ResponseType& rt);

///
/// The various HTTP response codes
///
enum ResponseCode : int {
       UNKNOWN_CODE                  = 0,
       CONTINUE                      = 100,
       OK                            = 200,
       CREATED                       = 201,
       ACCEPTED                      = 202,
       NO_CONTENT                    = 204,
       RESET_CONTENT                 = 205,
       PARTIAL_CONTENT               = 206,
       MULTIPLE_CHOICES              = 300,
       MOVED_PERMANENTLY             = 301,
       FOUND                         = 302,
       SEE_OTHER                     = 303,
       NOT_MODIFIED                  = 304,
       USE_PROXY                     = 305,
       TEMPORARY_REDIRECT            = 307,
       BAD_REQUEST                   = 400,
       UNAUTHORIZED                  = 401,
       PAYMENT_REQUIRED              = 402,
       FORBIDDEN                     = 403,
       NOT_FOUND                     = 404,
       METHOD_NOT_ALLOWED            = 405,
       NOT_ACCEPTABLE                = 406,
       PROXY_AUTHENTICATION_REQUIRED = 407,
       REQUEST_TIMEOUT               = 408,
       CONFLICT                      = 409,
       GONE                          = 410,
       LENGTH_REQUIRED               = 411,
       PRECONDITION_FAILED           = 412,
       REQUEST_ENTITY_TOO_LARGE      = 413,
       REQUEST_URI_TOO_LONG          = 414,
       UNSUPPORTED_MEDIA_TYPE        = 415,
       REQUEST_RANGE_BAD             = 416,
       EXPECTATION_FAILED            = 417,
       INTERNAL_SERVER_ERROR         = 500,
       NOT_IMPLEMENTED               = 501,
       BAD_GATEWAY                   = 502,
       SERVICE_UNAVAILABLE           = 503,
       GATEWAY_TIMEOUT               = 504,
       HTTP_VERSION_NOT_SUPPORTED    = 505
};


std::ostream& operator << (std::ostream& os, const ResponseCode& rc);
std::string& operator +(std::string& s, const ResponseCode& rc);
//std::string& operator+(const char* orig,const ResponseCode& rc);

const std::string translate(const ResponseCode& rt);

/*
class ResponseCode {
public:
  static std::string Translate(const ResponseCodes& val) {
    std::string result = "No such code";
    switch (val) {
    case ResponseCodes::UNKNOWN:
      result = "Unknown"; break;
    case ResponseCodes::CONTINUE:
      result = "Continue"; break;
    case ResponseCodes::OK:
      result = "OK"; break;
    case ResponseCodes::CREATED:
      result = "Created"; break;
    case ResponseCodes::ACCEPTED:
      result = "Accepted"; break;
    case ResponseCodes::NO_CONTENT:
      result = "No Content"; break;
    case ResponseCodes::RESET_CONTENT:
      result = "Reset Content"; break;
    case ResponseCodes::PARTIAL_CONTENT:
      result = "Partial Content"; break;
    case ResponseCodes::MULTIPLE_CHOICES:
      result = "Multiple Choices"; break;
    case ResponseCodes::MOVED_PERMANENTLY:
      result = "Moved Permanently"; break;
    case ResponseCodes::FOUND:
      result = "Found"; break;
    case ResponseCodes::SEE_OTHER:
      result = "See Other"; break;
    case ResponseCodes::NOT_MODIFIED:
      result = "Not Modified"; break;
    case ResponseCodes::USE_PROXY:
      result = "Use Proxy"; break;
    case ResponseCodes::TEMPORARY_REDIRECT:
      result = "Temporary Redirect"; break;
    case ResponseCodes::BAD_REQUEST:
      result = "Bad Request"; break;
    case ResponseCodes::UNAUTHORIZED:
      result = "Unauthorized"; break;
    case ResponseCodes::PAYMENT_REQUIRED:
      result = "Payment Required"; break;
    case ResponseCodes::FORBIDDEN:
      result = "Forbidden"; break;
    case ResponseCodes::NOT_FOUND:
      result = "Not Found"; break;
    case ResponseCodes::METHOD_NOT_ALLOWED:
      result = "Method Not Allowed"; break;
    case ResponseCodes::NOT_ACCEPTABLE:
      result = "Not Acceptable"; break;
    case ResponseCodes::PROXY_AUTHENTICATION_REQUIRED:
      result = "Proxy Authentication Required; "; break;
    case ResponseCodes::REQUEST_TIMEOUT:
      result = "Request Timeout"; break;
    case ResponseCodes::CONFLICT:
      result = "Conflict"; break;
    case ResponseCodes::GONE:
      result = "Gone"; break;
    case ResponseCodes::LENGTH_REQUIRED:
      result = "Length Required"; break;
    case ResponseCodes::PRECONDITION_FAILED:
      result = "Precondition Failed"; break;
    case ResponseCodes::REQUEST_ENTITY_TOO_LARGE:
      result = "Request Entity Too Large"; break;
    case ResponseCodes::REQUEST_URI_TOO_LONG:
      result = "Request URI Too Long"; break;
    case ResponseCodes::UNSUPPORTED_MEDIA_TYPE:
      result = "Unsupported Media Type"; break;
    case ResponseCodes::REQUEST_RANGE_BAD:
      result = "Request Range Not Satisfiable"; break;
    case ResponseCodes::EXPECTATION_FAILED:
      result = "Expectation Failed"; break;
    case ResponseCodes::INTERNAL_SERVER_ERROR:
      result = "Internal Server Error"; break;
    case ResponseCodes::NOT_IMPLEMENTED:
      result = "Not Implemented"; break;
    case ResponseCodes::BAD_GATEWAY:
      result = "Bad Gateway"; break;
    case ResponseCodes::SERVICE_UNAVAILABLE:
      result = "Service Unavailable"; break;
    case ResponseCodes::GATEWAY_TIMEOUT:
      result = "Gateway Timeout"; break;
    case ResponseCodes::HTTP_VERSION_NOT_SUPPORTED:
      result = "HTTP Version Not Supported"; break;
    }
    return result;
  };
};
*/

}

#endif /* defined(RESPONSECODES_H) */
